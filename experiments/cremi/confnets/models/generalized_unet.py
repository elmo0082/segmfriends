...
class MultiScaleInputMultiOutputUNet(EncoderDecoderSkeleton):
    ...
    def forward(self, *inputs):
        ...
        #np.savetxt('pic.csv', picture[0,:,:].cpu().detach().numpy())

        #layer = LevelSetLayer2D(size=(272, 272), maxdim=1, sublevel=False)
        #dgms = layer(outputs)
    ...
    def construct_upsampling_module(self, depth):
        # First we need to reduce the number of channels:
...
class MultiScaleInputUNet(MultiScaleInputMultiOutputUNet):
    def __init__(self,
        ...
        if final_activation is not None:
            output_branches_specs[0]["activation"] = final_activation
        if final_normalization is not None:
            output_branches_specs[0]["normalization"] = final_normalization
        if final_nb_norm_groups is not None:
            output_branches_specs[0]["nb_norm_groups"] = final_nb_norm_groups
